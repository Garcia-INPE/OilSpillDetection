...
"""02.2-Adjust_preprocessed_windows.py

Manual window editing toolbox for a single image.

This script is intentionally stateful through JSON updates:
- It reads/writes dataout/02.1-Preprocessed_windows_in_800x600.json by default.
...
def _default_json_path(width, height):
    """Build default JSON path suffixed with window size.
    ...
    Returns:
        str: Default JSON path under dataout.
    """
    return os.path.join("dataout", f"02.1-Preprocessed_windows_in_{width}x{height}.json")
...
def _default_out_dir(width, height):
    ...
    """
    return os.path.join("dataout", f"02.2-POLYGONS_BY_IMAGE_{width}x{height}")
...
def _load_024_module():
    """Load the 02.4 plotting script as an importable module.
    ...
    spec = importlib.util.spec_from_file_location(
        "plot_024",
        "02.4-PlotPolygonsByImage_800x600.py",
    )
...
def _window_start_from_geometry_edge(window_edge, seg_min, seg_max, window_size, axis_limit):
    ...
    if window_edge in {"left", "top"}:
        # Window's left/top edge sits 1 pixel before the geometry's min bound
        start = int(seg_min) - 1
    else:
        # Window's right/bottom edge sits 1 pixel after the geometry's max bound
        start = int(seg_max) + 1 - int(window_size)
...
def plot_current_image(overwrite=True):
    """Render a QA plot for the current image and window layout.
...
def plot_global_bounding_box_zoom(
    ...
    """Plot global bounding-box zoom for selected windows/geometries.
...
def center_window_on_geometries(window_id, poly_ids_1b):
    """Move an existing window to the center of selected geometries.
...
def align_window_x(window_id, ref_window_id, my_edge, ref_edge, shift_px=0):
    """Align one window horizontally to another by edge pairing.
...
def align_window_y(window_id, ref_window_id, my_edge, ref_edge, shift_px=0):
    """Align one window vertically to another by edge pairing.
...
def shift(window_id, axis, amount):
    """Shift one existing window on one axis by a pixel amount.
...
def snap_window_edge_to_geometry_bound(window_id, poly_id_1b, window_edge, geometry_bound):
    """Move an existing window by snapping one edge to one geometry bound.
...
def create_window_at_row_col(row_off, col_off):
    """Create a new window at explicit pixel offsets.

    The requested offsets are clamped to image bounds.

    Args:
        row_off (int): Proposed top row offset in pixels.
        col_off (int): Proposed left column offset in pixels.
    ...
    src, gdf = _ensure_src_and_gdf(IMAGE_ID)
    try:
        row, col = _clamp_window(row_off, col_off, src)
...
def create_window_centered_on_geometries(poly_ids_1b):
    """Create a new window centered on selected geometries.
...
def create_window_adjacent_to_window(ref_window_id, side, align="center"):
    """Create a new window adjacent to a reference window.
...
def create_window_by_geometry_bound(poly_id_1b, window_edge, geometry_bound):
    """Create a new window by matching one edge to a geometry bound.
...
def check_window_geometries(window_id):
    """Check containment/intersection status for one window.
...
def check_geometry_in_windows(poly_id_1b):
    """Check whether one geometry is fully inside a window or window union.
...
def check_image_coverage_and_commit_if_ok(expected_width=800, expected_height=600, commit_if_ok=False):
    """Check global coverage and commit memberships only when valid.
...
def reindex_windows():
    """Reindex windows to contiguous keys while preserving key order.
...
def delete_window_and_reindex(window_id):
    """Delete one window by ID and reindex remaining windows contiguously.
...
def list_toolbox_functions(print_output=True, compact=True):
    """Return available toolbox functions and optionally print them.
    ...
    if compact:
        ...
        lines = [
            "Creation functions:",
            "- create_window_at_row_col(row_off, col_off)",
...
def _terminal_quick_guide():
    ...
    return "\n".join(
        [
            ...
            "  list                    Show available toolbox functions",
            "  list --full             Show complete function signatures",
            "  show-doc NAME           Show docstring for one function",
            "  show-doc --all          Show docstrings for all public functions",
            "",
            "All commands:",
            "  list                    List available toolbox functions",
            "  plot                    Render current QA plot",
            "  plot-bbox               Render global bbox zoom (00-GLOBAL_BBOX_ZOOM + optional 00-CURRENT_FOCUS_WINDOW)",
            "  check-window ID         Check one window geometry status",
            "  check-geometry ID       Check if one geometry is fully inside window(s) or their union",
            "  check-image             Check image coverage (no write by default)",
            "  create-at R C           Create window at row/col",
            "  create-centered IDS     Create window centered on geometries",
            "  create-adjacent ...     Create window adjacent to reference",
            "  create-bound ...        Create window by geometry bound",
            "  move-center ...         Center an existing window on geometries",
            "  align-x ...             Align window horizontally",
            "  align-y ...             Align window vertically",
            "  shift ...               Shift window on axis x/y",
            "  snap ...                Snap window edge to geometry bound",
            "  delete-window ID        Delete one window and reindex",
            "  reindex                 Reindex window keys contiguously",
            "  show-doc NAME           Show complete docstring for one function",
            "  show-doc --all          Show all function docstrings",
            "",
            "Global options:",
            ...
            "  --confirm-before-change Ask confirmation before mutating commands (default)",
            "  --no-confirm-before-change Disable confirmation for mutating commands",
            "",
            "Example:",
...
def _doc_examples_for_function(function_name):
    ...
    examples = {
        ...
        "create_window_at_row_col": [
            (f"{script} create-at 100 200", "Create a new window with top-left at row 100, col 200."),
        ],
        "create_window_centered_on_geometries": [
...
def _apply_global_overrides(image_id=None, width=None, height=None, focus_margin=None, json_path=None, out_dir=None):
    ...
    """
    global IMAGE_ID, WINDOW_WIDTH, WINDOW_HEIGHT, FOCUS_MARGIN, JSON_PATH, DIR_OUT
...
def _doc_alias_to_function_name(alias):
    ...
    mapping = {
        ...
        "create-at": "create_window_at_row_col",
...
def _build_cli_parser():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  02.5-ManualWindowTools.py show-doc create_window_at_row_col\n"
            "  02.5-ManualWindowTools.py --image-id 7 --width 800 --height 600 check-image\n"
    ...
    confirm_group.add_argument(
        ...
        action="store_true",
        help="Ask confirmation before commands that may change image windows/JSON (default)",
    )
    confirm_group.add_argument(
        "--no-confirm-before-change",
        dest="confirm_before_change",
        action="store_false",
        help="Run mutating commands without Y/N confirmation",
    )
    parser.set_defaults(confirm_before_change=True)
    parser.add_argument(
        "--show-settings",
        action="store_true",
    ...
    p_list = sub.add_parser("list", help="List available toolbox functions")
    p_list.add_argument("--full", action="store_true", help="Show full signatures")

    sub.add_parser("plot", help="Render current QA plot")

    p_plot_bbox = sub.add_parser(
        "plot-bbox",
        help="Render global bbox zoom and save image-named output",
    )
    ...
    p_check_w = sub.add_parser("check-window", help="Check one window geometry status")
    p_check_w.add_argument("window_id", type=int)

    p_check_g = sub.add_parser(
        "check-geometry",
        help="Check if one geometry is fully inside one or more windows or their union",
    )
    p_check_g.add_argument("poly_id", type=int)

    p_check_i = sub.add_parser("check-image", help="Check image coverage (optional commit when requested)")
    ...
    p_create_at = sub.add_parser("create-at", help="Create window at row/col")
    p_create_at.add_argument("row_off", type=int)
    p_create_at.add_argument("col_off", type=int)

    p_create_center = sub.add_parser("create-centered", help="Create window centered on geometries")
    p_create_center.add_argument("poly_ids", help="Comma-separated poly IDs, e.g. 1,2,3")

    p_create_adj = sub.add_parser("create-adjacent", help="Create window adjacent to reference window")
    p_create_adj.add_argument("ref_window_id", type=int)
    p_create_adj.add_argument("side", choices=["east", "west", "north", "south"])
    p_create_adj.add_argument("--align", default="center")

    p_create_bound = sub.add_parser("create-bound", help="Create window by geometry bound")
    p_create_bound.add_argument("poly_id", type=int)
    p_create_bound.add_argument("window_edge", choices=["left", "right", "top", "bottom"])
    p_create_bound.add_argument("geometry_bound", choices=["west", "east", "north", "south"])

    p_move_center = sub.add_parser("move-center", help="Center an existing window on geometries")
    p_move_center.add_argument("window_id", type=int)
    p_move_center.add_argument("poly_ids", help="Comma-separated poly IDs, e.g. 1,2,3")

    p_align_x = sub.add_parser("align-x", help="Align window horizontally to reference")
    p_align_x.add_argument("window_id", type=int)
    p_align_x.add_argument("ref_window_id", type=int)
    p_align_x.add_argument("my_edge", choices=["left", "right"])
    p_align_x.add_argument("ref_edge", choices=["left", "right"])
    ...
    p_align_y = sub.add_parser("align-y", help="Align window vertically to reference")
    p_align_y.add_argument("window_id", type=int)
    p_align_y.add_argument("ref_window_id", type=int)
    p_align_y.add_argument("my_edge", choices=["top", "bottom"])
    p_align_y.add_argument("ref_edge", choices=["top", "bottom"])
    ...
    p_shift = sub.add_parser("shift", help="Shift existing window on axis x/y")
    ...
    p_snap = sub.add_parser("snap", help="Snap existing window edge to geometry bound")
    p_snap.add_argument("window_id", type=int)
    p_snap.add_argument("poly_id", type=int)
    p_snap.add_argument("window_edge", choices=["left", "right", "top", "bottom"])
    p_snap.add_argument("geometry_bound", choices=["west", "east", "north", "south"])

    p_delete_window = sub.add_parser("delete-window", help="Delete one window and reindex remaining windows")
    p_delete_window.add_argument("window_id", type=int)

    sub.add_parser("reindex", help="Reindex window keys contiguously")

    p_show_doc = sub.add_parser("show-doc", help="Show complete docstring for one function")
    p_show_doc.add_argument(
        "function_name",
        type=str,
        nargs="?",
        help="Function name, for example create_window_at_row_col",
    )
    p_show_doc.add_argument(
        "--all",
        action="store_true",
...
def _run_cli(argv):
    ...
    if len(argv) == 1 and not argv[0].startswith("-"):
        ...
        no_arg_alias_commands = {"list", "plot", "check-image", "reindex"}
    ...
    if args.command == "check-image":
        ...
        if outputs["current_focus"]:
            print(outputs["current_focus"])
        return 0
    if args.command == "create-at":
        new_id = create_window_at_row_col(args.row_off, args.col_off)
        print(new_id)
        outputs = render_all_outputs_for_current_image(
            focus_window_ids=[new_id],
            focus_command_name="create-at",
        )
        print(outputs["qa"])
        print(outputs["image_named"])
        if outputs["current_focus"]:
    ...
    if args.command == "create-adjacent":
        new_id = create_window_adjacent_to_window(args.ref_window_id, args.side, align=args.align)
        print(new_id)
        outputs = render_all_outputs_for_current_image(
    ...
    if args.command == "snap":
        snap_window_edge_to_geometry_bound(args.window_id, args.poly_id, args.window_edge, args.geometry_bound)
        outputs = render_all_outputs_for_current_image(
            focus_window_ids=[args.window_id],
            focus_poly_ids=[args.poly_id],
