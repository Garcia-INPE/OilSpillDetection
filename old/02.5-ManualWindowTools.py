...
- It reads window positions from dataout/02.1-Created_windows_in_800x600.json (source,
  read-only) as a starting point.
- All adjustments are saved to a separate dataout/02.2-Adjusted_windows_in_800x600.json so
  the 02.1 output is never modified.
...
def _default_json_in_path(width, height):
    ...
    """
    return os.path.join("dataout", f"02.1-Created_windows_in_{width}x{height}.json")
...
def _default_json_path(width, height):
    ...
    """
    return os.path.join("dataout", f"02.2-Adjusted_windows_in_{width}x{height}.json")
...
def _default_out_dir(width, height):
    ...
    """
    return os.path.join("dataout", f"02.1.2-Adjusted_windows_{width}x{height}")
...
def _load_024_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "plot_024",
        "02.1.1-PlotPolygons_800x600.py",
    )
