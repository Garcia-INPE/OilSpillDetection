...
DIR_OUT = os.path.join('dataout', '02.4-POLYGONS_BY_IMAGE')
...
TARGET_W = 800
TARGET_H = 600
MANUAL_WINDOWS_JSON = os.path.join('dataout', f'02.1-Preprocessed_windows_in_{TARGET_W}x{TARGET_H}.json')
...
def plot_image_polygons_800x600(tiff_path, gdf_img, out_png, gdf_land):
    """Render a PNG with polygons, optional land overlay, and 800x600 windows."""
    with rasterio.open(tiff_path) as src:
        ...
        if len(gdf_plot) > 0:
            ...
            if image_id in MANUAL_WINDOWS_BY_IMAGE:
                windows = MANUAL_WINDOWS_BY_IMAGE[image_id]
                covered_idx = set()
                for window in windows:
                    covered_idx |= window['covered']
                uncovered_idx = set(range(len(gdf_plot))) - covered_idx
            else:
                windows, covered_idx, uncovered_idx = select_non_overlapping_windows(src, gdf_plot, TARGET_W, TARGET_H)
            win_color = '#8A2BE2'
            centroid_points = [
            ...
            for i, w in enumerate(windows, start=1):
                left, bottom, right, top = window_bounds_geo(src, w['row_off'], w['col_off'], TARGET_W, TARGET_H)
                ax.add_patch(
                    Rectangle(
                        (left, bottom),
                        right - left,
...
def main():
    """Generate 800x600 polygon plots for all configured TIFF images."""
    id_to_tiff = build_id_to_tiff_path(Cfg.FNAMES_TIF)
    gdf_land = gpd.read_file(LAND_PATH)

    print(f'[INFO] Creating *POLYGONS_800x600.png files in: {DIR_OUT}')
    ...
    for id_tiff, tiff_path in sorted(id_to_tiff.items()):
        ...
        out_png = os.path.join(DIR_OUT, f'IMG_{id_tiff:02d}_POLYGONS_800x600.png')
        plot_image_polygons_800x600(tiff_path, gdf_img, out_png, gdf_land)
        generated += 1
