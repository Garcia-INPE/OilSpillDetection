...
PLOT = True
DIR_IMG = "./dataout/img"
DIR_DATA = f"{HOME}/ProjData/Oil_Spill/Cantarell_Beisl"
DIR_TIF = f"{DIR_DATA}/Calibrada/" if BITS == 16 else f"{DIR_DATA}/8bits/"
FNAME_CSV = f"dataout/Oil_Stats_{BITS}bits.csv"
...
with open(FNAME_CSV, 'w', encoding='utf-8') as f1:
    ...
    for tiff_idx, tiff_fname in enumerate(FNAMES_TIF):
        ...
        gdf_img_mpolys = VETORES.columns[VETORES['Id'] == id_tiff]
        ...
        idx_poly = 0
        for idx_poly in range(len(gdf_img_mpolys)):
            ...
            dict_feat_stat_pol = Stat.get_feat_stat(
                gdf_img_mpolys, gdf_poly, DICT_FEAT_STAT, tiff_file, True, DIR_IMG)
